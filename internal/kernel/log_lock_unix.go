...
// kernel. Per-OFD semantics keep this lock independent of the store's
// per-Append opens (which never flock), and make a second acquire — from any
// process, including this one — fail fast. The OS releases the lock when the
// owning process exits: crash-safe, no stale locks. See moos-kernel#40 for
// the multi-writer failure this prevents.
